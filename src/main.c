...
#define LED_AMARELO_NODE  DT_ALIAS(led1)
...
#if !DT_NODE_HAS_STATUS(LED_AMARELO_NODE, okay)
...
static const struct gpio_dt_spec led_amarelo =
    GPIO_DT_SPEC_GET(LED_AMARELO_NODE, gpios);
...
static void semaforo_set(int verde, int amarelo, int vermelho)
{
    gpio_pin_set_dt(&led_verde, verde);
    gpio_pin_set_dt(&led_amarelo, amarelo);
